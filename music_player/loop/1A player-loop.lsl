...
upload_music(string music){if((key)music){music_song = []; music_song += (list)[music]; playmusic();}}
playmusic()
{ 
  list a =llGetLinkPrimitiveParams(2,[PRIM_DESC]);    
  list items = llParseString2List(llList2String(a,0),["="],[]);
  llLinkSetSoundRadius(LINK_THIS,llList2Float(items,1));
  if(music_selection == "none"){return;}
  if((key)llList2String(music_song,0))
  {
  llLoopSound(llList2String(music_song,0),llList2Float(items,0));
  }else{
...
default
{
    ...
    if(llList2String(item, 0) == "memory_result"){secondarymemory = llList2Integer(item, 1);songlist = llListSort(songlist,1, TRUE);}
    ...
    if(skey == llGetOwner()) 
    {
      ...
      if(text == "[ Play ]"){playmusic(); dialog_topmenu();}
      if(text == "[ Pause ]"){llStopSound(); dialog_topmenu();}
